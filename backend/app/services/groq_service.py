...
Please provide a detailed, clear explanation that's easy for students to understand. Explain the concept well and add citations [Source 1], [Source 2], etc. after key points."""
    ...
    def _extract_citations(
        ...
        """Extract citations from retrieved chunks with accurate page numbers"""
        citations = []
        
        # Only take the TOP 1 most relevant chunk for citation
        if not chunks:
            return citations
            
        chunk = chunks[0]  # Most relevant chunk
        meta = chunk.get("metadata", {}) or {}
        
        # Get accurate page information from pages array
        pages = meta.get("pages", [])
        if pages and len(pages) > 0:
            # Use the first page's page_number as the primary citation
            ...
            # Fallback to primary_page or metadata page
            primary_page = str(chunk.get('primary_page') or meta.get("page", "Unknown"))
        ...
        # Get chapter - avoid duplication if metadata already has "Chapter"
        ...
        if chapter_from_meta and chapter_from_meta != "Unknown" and str(chapter_from_meta).strip():
            ...
            # Extract chapter from filename (e.g., jehp101 -> Chapter 1)
            match = re.search(r'(jehp|jemh|jeff|jesc|jess)(\d+)', filename)
            if match:
                chapter_num = str(int(match.group(2)) % 100)  # Get chapter number
        ...
        # Create single citation from most relevant chunk
        citation = Citation(
